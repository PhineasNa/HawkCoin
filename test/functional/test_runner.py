...
def main():
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and hawkcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    ...
    try:
        if subprocess.check_output(["pidof", "hawkcoind"]) is not None:
            print("%sWARNING!%s There is already a hawkcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "LITECOIND" not in os.environ:
        os.environ["LITECOIND"] = build_dir + '/src/hawkcoind' + exeext
