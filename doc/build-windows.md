...
Below are some notes on how to build Hawkcoin Core for Windows.

The options known to work for building Hawkcoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Hawkcoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Hawkcoin Core applications.
...
Note that for WSL the Hawkcoin Core source path MUST be somewhere in the default mount file system, for
