Hawkcoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Hawkcoin transactions now use validation using
...
Previous versions of Hawkcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Hawkcoin Core was to raise the default minimum
relay fee.

Hawkcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Hawkcoin Core 0.13.2 also introduces new default policy limits on the length and
...
Hawkcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Hawkcoin Core will relay transactions with
...
entirely in Hawkcoin Core 0.13, and it is at this time undecided whether the
...
Hawkcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Hawkcoin Core automatically creates a hidden service to listen on, without
manual configuration. Hawkcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Hawkcoin Core is listening, and
...
Hawkcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Hawkcoin
Core to use floating fees. By default, Hawkcoin Core will use floating
fees.
...
(default: `0.0002` HWK/kB).

At all times, Hawkcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) HWK.
Furthermore, Hawkcoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Hawkcoin Core code base has
...
for a new opcode in the Hawkcoin scripting system that in combination with
...
Hawkcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Hawkcoin Core.
...
The two phases are distinct in the Hawkcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Hawkcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Hawkcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Hawkcoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Hawkcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Hawkcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Hawkcoin's current ledger (called the UTXO set),
  ...
  note that Hawkcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Hawkcoin users to individually opt-in to almost any change in the Hawkcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Hawkcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Hawkcoin
client to Hawkcoin Core 0.13.2 to signal for segwit support. When a
...
Since Hawkcoin Core 0.10.0, nodes have defaulted to only relaying and
