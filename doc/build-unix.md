UNIX BUILD NOTES
====================
Some notes on how to build Hawkcoin Core in Unix.
...
memory available when compiling Hawkcoin Core. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build Hawkcoin Core without wallet.
...
If you want to build Hawkcoin-Qt, make sure that the required packages for Qt development
...
# Configure Hawkcoin Core to use our own-built instance of BDB
cd $LITECOIN_ROOT
./autogen.sh
...
As mentioned above, when maintaining portability of the wallet between the standard Hawkcoin Core distributions and independently built
...
with 4.8-built Hawkcoin Core is needed follow the steps under "Berkeley DB" above.
